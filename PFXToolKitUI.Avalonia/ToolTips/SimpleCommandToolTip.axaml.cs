...
public partial class SimpleCommandToolTip : UserControl, IToolTipControl {
    ...
    static SimpleCommandToolTip() {
        TextProperty.Changed.AddClassHandler<Control, string?>((s, e) => OnTextOrCommandIdChanged(s));
        CommandIdProperty.Changed.AddClassHandler<Control, string?>((s, e) => OnTextOrCommandIdChanged(s));
    }
    ...
    private static void OnTextOrCommandIdChanged(Control control) {
    ...
    public void OnOpened(Control owner, IContextData data) {
        ...
        if (KeymapUtils.TryGetStringForCommandId(GetCommandId(owner), out string? gesture)) {
