...
public class NumberDragger : RangeBase {
    ...
    private void UpdateCursor() {
        StandardCursorType cursor = this.dragState != 2 ? StandardCursorType.Arrow : DragDirectionToCursor(this.DragDirection);
    ...
    private bool CompleteEdit(Key inputKey, bool isCompletingFromLostFocus) {
        ...
        try {
            if (this.IsIntegerValue) {
                ...
                context.RegisterVariable("v", (long) Math.Round(this.Value));
                ...
                context.RegisterVariable("v", this.Value);
                parsedValue = evaluator(context);
            }
        }
        catch {
