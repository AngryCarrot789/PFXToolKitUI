...
public static class EventUtils {
    ...
    public static Delegate CreateDelegateToInvokeActionFromEvent(Type eventHandlerType, Action actionToInvoke) {
        ...
        if (!EventToParamListMap.TryGetValue(eventHandlerType, out ParameterExpression[]? paramArray)) {
            MethodInfo invokeMethod = eventHandlerType.GetMethod("Invoke") ?? throw new Exception(eventHandlerType.Name + " is not a delegate type");
            EventToParamListMap[eventHandlerType] = paramArray = invokeMethod.GetParameters().Select(p => Expression.Parameter(p.ParameterType, p.Name)).ToArray();
        }
