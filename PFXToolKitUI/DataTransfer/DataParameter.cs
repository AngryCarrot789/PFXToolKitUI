...
public abstract class DataParameter : IEquatable<DataParameter>, IComparable<DataParameter> {
    ...
    public static void SetValueHelper<T>(ITransferableData owner, DataParameter<T> parameter, ref T field, T newValue) {
        ...
        else if (!EqualityComparer<T>.Default.Equals(field, newValue)) {
...
public class DataParameter<T> : DataParameter {
    private readonly ValueAccessor<T>? accessor; // when null, we use TransferableData's internal storage
    ...
    private class DataParameterValueAccessor : ValueAccessor<T> {
        ...
        public DataParameterValueAccessor(DataParameter<T> parameter) {
    ...
    public virtual T GetValue(ITransferableData owner) {
        if (this.accessor == null)
            return (T) owner.TransferableData.InternalGetStorageValue(this)!;
        return this.accessor.GetValue(owner)!;
    }

    public override object? GetObjectValue(ITransferableData owner) {
        if (this.accessor == null)
            return owner.TransferableData.InternalGetStorageValue(this);
    ...
    public virtual void SetValue(ITransferableData owner, T value) {
        ...
        try {
            if (this.accessor == null) {
                owner.TransferableData.InternalSetStorageValue(this, value);
            }
            else {
