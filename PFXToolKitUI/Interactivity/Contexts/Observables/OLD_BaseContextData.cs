...
/// </summary>
// public abstract class OLD_BaseContextData : IMutableContextData {
...
//             if (this.Count < 1 || !(dict = this.InternalDictionary).TryGetValue(key, out object? existing)) {
//                 (dict ?? this.InternalDictionary)[key] = value;
//                 this.ContextChanged?.Invoke(this, new ContextChangedEventArgs([KVP(key, value)], Span<KeyValuePair<string, object>>.Empty));
//             }
//             else if (!Equals(existing, value)) {
//                 dict[key] = value;
//                 this.ContextChanged?.Invoke(this, new ContextChangedEventArgs([KVP(key, value)], [KVP(key, existing)]));
//             }
//         }
//     }
//
...
//         }
//         else if (this.Count > 0 && this.InternalDictionary.Remove(key, out object? existing)) {
//             this.ContextChanged?.Invoke(this, new ContextChangedEventArgs(Span<KeyValuePair<string, object>>.Empty, [KVP(key, existing)]));
//         }
//     }
//
...
//         }
//
//         ContextChangedEventHandler? changeHandler = this.ContextChanged;
//         if (changeHandler != null) {
//             this.ProcessBatchesWithChangeEvent(list, changeHandler);
//         }
//         else {
//             this.ProcessBatchesWithoutChangeEvent(list);
//         }
//
//         this.myBatchModifications = null;
//     }
//
//     private void ProcessBatchesWithChangeEvent(List<ModificationEntry> list, ContextChangedEventHandler changeHandler) {
//         int added = 0, removed = 0;
//         using RentHelper.Token<KeyValuePair<string, object>> _ = RentHelper.RentSpan(list.Count, out Span<KeyValuePair<string, object>> span);
...
//                 if (!map.TryGetValue(entry.Key, out object? existing)) {
...
//                 }
//
//                 // set added entries at the start of the list, heading towards the end
//                 span[added++] = new KeyValuePair<string, object>(entry.Key, entry.Value!);
//             }
//             else if (map.Remove(entry.Key, out object? existingValue)) {
//                 // set removed entries at the end of the list, heading towards the start
//                 span[span.Length - 1 - removed++] = new KeyValuePair<string, object>(entry.Key, existingValue);
//             }
//         }
//
//         if (added > 0 || removed > 0) {
//             changeHandler(this, new ContextChangedEventArgs(span.Slice(0, added), span.Slice(span.Length - removed)));
//         }
//     }
//
//     private void ProcessBatchesWithoutChangeEvent(List<ModificationEntry> list) {
//         IDictionary<string, object> map = this.InternalDictionary;
//         foreach (ModificationEntry entry in list) {
//             if (entry.IsAdding) {
//                 Debug.Assert(entry.Value != null, "Insertion entry's value should not be null");
...
//     #endregion
//
//     private static KeyValuePair<string, object> KVP(string key, object existing) {
//         return new KeyValuePair<string, object>(key, existing);
//     }
//
